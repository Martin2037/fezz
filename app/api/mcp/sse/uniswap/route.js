...
import {
  ...
  getRaydiumSwap,
  getRoute,
} from "@/app/api/mcp/sse/uniswap/lib";
...
server.tool(
  ...
  "Swap cryptocurrency tokens across multiple blockchain networks with real-time rates and optimal routing. This tool enables seamless token exchanges between Ethereum, BSC, Base, and Arbitrum networks except Solana, supporting both native coins (ETH, BNB) and any ERC-20/BEP-20 tokens. The source token always be native token, so don't ask user to provide it.",
  {
    ...
    dstTokenAddress: z
      ...
      .length(42)
      .describe(
        "The token address that user want to trade[if you dont know, you must ask user to provide! Use context to get address such as the last tool result.] [BUSD stand for 0x55d398326f99059ff775485246999027b3197955, don't ask user to provide!]",
      ),
    chainId: z
      .enum(["1", "56", "8453", "42161"])
      .describe(
        "链ID，支持的值: 1(以太坊), 56(BSC/BNB), 137(Polygon), 8453(Base), 42161(Arbitrum) [if you dont know, you must ask user to provide!] [BSC(56) Native token is BNB, so don't ask user to provide!]",
      ),
    amount: z
      .number()
      .gt(0)
      .describe(
        "The token amount that user wants to trade [if you dont know, you must ask user to provide!]",
      ),
  },
...
//       .length(44)
//       .describe(
//         "The token address that user want to trade[if you dont know, you must ask user to provide! Use context to get address such as the last tool result.]",
//       ),
//     amount: z
//       .number()
//       .gt(0)
...
server.tool(
  "raydium_swap",
  "Swap cryptocurrency tokens through Raydium amm swap. This tool only support Solana chain.",
  {
    // walletAddress: z.string().length(42).describe('The user wallet address that will be used to perform the swap [if you dont know, you must ask user to provide!]'),
    // srcTokenAddress: z.string().length(42).describe('The source token address that user want to trade, if token is native token like ETH,BNB, the address is 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee [if you dont know, you must ask user to provide! Use context to get address such as the last tool result]'),
    dstTokenAddress: z
  ...
  async ({ dstTokenAddress, amount }) => {
    try {
      const walletAddress = "AYLhUF7jsNZU8bdVKEVQYmnXnM8m7GimEQ7xHFYNnnPc";
      const data = await getRaydiumSwap(walletAddress, dstTokenAddress, amount);
    ...
    new ReadableStream({
      start(controller) {
        try {
          ...
          // 创建 SSE 传输层
          transport = new NextSSEServerTransport(
            "/api/mcp/sse/uniswap",
            expressRes,
          );
          ...
          server.connect(transport).catch((error) => {
            ...
            controller.error(
              new Error(`Failed to connect to MCP server: ${error.message}`),
            );
          });
